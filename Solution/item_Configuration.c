...
void door_Placement(unsigned char *_map_Array, unsigned char *_map_Index_Array, unsigned char *_room_Height_Array, unsigned char *_room_Width_Array, int _current_Position_On_Map, int _returned_Map_Width/*, int _size_Of_Map*/){
    ...
    if(_map_Array[_map_Index_Array[_current_Position_On_Map] + _returned_Map_Width] != NULL){ 
        ...
        // Goal: Figure out a way to grab the targeted map in the array.
        while(_map_Array[_map_Index_Array[_current_Position_On_Map] + _returned_Map_Width] != blank_Tile[0]){
            returned_X_Value = randw() % _room_Width_Array[_current_Position_On_Map]; returned_Y_Value = randw() % _room_Height_Array[_current_Position_On_Map]; 
            position_Marker = _room_Width_Array[_current_Position_On_Map] * returned_Y_Value + returned_X_Value;
        }
